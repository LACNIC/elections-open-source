		...
		try {
			...
			while (i < hoja.getRows() && !hoja.getCell(0, i).getContents().equals("")) {
				...
				if (!(EmailValidator.getInstance().isValid(mail)))
					throw new ValidacionPadronException("Fila: " + i + " no contiene un e-mail valido: " + mail);
				if(!mails.add(mail))
					throw new ValidacionPadronException("No puede haber dos votantes con el mismo e-mail, fila: " + i + " contiene un e-mail duplicado");
				...
				try {
					...
					throw new ValidacionPadronException("Fila: " + i + " no contiene una cantidad de votos válida: " + votos);
				}
				...
				if (indiceIdioma != -1) {
					...
					if (!idioma.matches("SP|EN|PT")) {
						throw new ValidacionPadronException("Fila: " + i + " no contiene un idioma reconocido por el sistema: " + idioma + ", debe ser SP, EN ó PT");
					}
				...
				if (indicePais != -1) {
					...
					if (!pais.isEmpty() && !u.getIdPaises().contains(pais)) {
						throw new ValidacionPadronException("Fila: " + i + " contiene un país no vacío y no válido: " + pais);
					}
