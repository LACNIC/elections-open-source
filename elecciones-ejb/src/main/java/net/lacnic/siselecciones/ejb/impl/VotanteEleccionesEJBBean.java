...
import net.lacnic.siselecciones.dao.UsuarioPadronDao;
...
import net.lacnic.siselecciones.utils.FileUtils;
...
public class VotanteEleccionesEJBBean implements VotanteEleccionesEJB {
	...
	@Override
	public void votar(List<Candidato> candidatos, UsuarioPadron up, String ip) throws OperacionNoPermitidaException {
		...
		TemplateEleccion t = EJBFactory.getInstance().getManagerEleccionesEJB().obtenerTemplate(Constantes.TipoTemplateCODIGOS_VOTACION, usP.getEleccion().getIdEleccion());
	...
	public void confirmarEleccionAuditor(long idAuditor) {
		try {
			...
			em.persist(a);
			TemplateEleccion t = EJBFactory.getInstance().getManagerEleccionesEJB().obtenerTemplate(Constantes.TipoTemplateAUDITOR_CONFORME, a.getEleccion().getIdEleccion());
	...
	public void habilitarRevisionEleccionAuditor(long idAuditor, String ip) {
		try {
			...
			em.persist(a);
			TemplateEleccion t = EJBFactory.getInstance().getManagerEleccionesEJB().obtenerTemplate(Constantes.TipoTemplateAUDITOR_REVISION, a.getEleccion().getIdEleccion());
			EJBFactory.getInstance().getEnvioMailsEJB().encolarEnvioIndividual(t, null, a, a.getEleccion(), new ArrayList<>());
