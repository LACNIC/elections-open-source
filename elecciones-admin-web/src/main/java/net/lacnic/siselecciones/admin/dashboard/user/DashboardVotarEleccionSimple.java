...
import net.lacnic.siselecciones.admin.panel.user.DetalleVoteEleccionSimplePanel;
...
public class DashboardVotarEleccionSimple extends DashboardPublicBasePage {
	...
	public DashboardVotarEleccionSimple(PageParameters params) {
		...
		if (upd.isYaVoto()) {
			...
			ListView<Candidato> candidatosDataView = new ListView<Candidato>("candidatosList", Contexto.getInstance().getManagerBeanRemote().obtenerCandidatosEleccionOrdenados(getEleccion().getIdEleccion())) {
	...
	public Class validarToken(PageParameters params) {
		...
		if ((token1 == null || token1.equals("")) && (token2 == null || token2.equals(""))) {
			...
			if (token1 == null || token1.equals(""))
				upd = Contexto.getInstance().getVotanteBeanRemote().verificarAccesoUP(token2);
			else
				upd = Contexto.getInstance().getVotanteBeanRemote().verificarAccesoUP(token1);
		}
	...
	public void initBoton() {
		...
		Link<Void> confirmar = new Link<Void>("confirmar") {
			...
			public void onClick() {
				if (isOkForVote()) {
					try {
						Contexto.getInstance().getVotanteBeanRemote().votar(getElegidos(), upd, getIP());
	...
	private boolean isOkForVote() {
		try {
			if (Contexto.getInstance().getVotanteBeanRemote().yaVoto(upd.getIdUsuarioPadron())) {
