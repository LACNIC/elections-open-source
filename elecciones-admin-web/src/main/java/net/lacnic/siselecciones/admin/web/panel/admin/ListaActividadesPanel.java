...
public class ListaActividadesPanel extends Panel {
	...
	public ListaActividadesPanel(String id, long idEleccion){
		super(id);
		if(idEleccion == -1)
			listaActividades = Contexto.getInstance().getManagerBeanRemote().obtenerTodasLasActividades();
		else
			listaActividades = Contexto.getInstance().getManagerBeanRemote().obtenerTodasLasActividades(idEleccion);
