...
public class DashboardEditarAuditor extends DashboardAdminBasePage {
	...
	public DashboardEditarAuditor(PageParameters params) {
		...
		auditor = Contexto.getInstance().getManagerBeanRemote().obtenerAuditor(idAuditor);
		...
		formAuditor.add(new Button("editar") {
			...
			public void onSubmit() {
				...
				if (!(email.equalsIgnoreCase(auditor.getMail())) || !(nombre.equalsIgnoreCase(auditor.getNombre()))) {
					Contexto.getInstance().getManagerBeanRemote().editarAuditor(getAuditor(), SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
