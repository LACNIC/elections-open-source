		...
		Form<Void> form = new Form<Void>("form") {
			...
			protected void onSubmit() {
				...
				if ((getTipoDestinatario() != null) && (getTipoDestinatario().compareTo(TipoDestinatario.VOTANTES) == 0)) {
					mailPadron = true;
					isSupra = Contexto.getInstance().getManagerBeanRemote().isSupraEleccion(template.getEleccion().getIdEleccion());
					// Si es elección conjunta y se va a enviar mail al padron, debeo validar si son iguales 
					if (isSupra) {
						supraElec = Contexto.getInstance().getManagerBeanRemote().obtenerSupraEleccion(template.getEleccion().getIdEleccion());
						padronIgual = Contexto.getInstance().getManagerBeanRemote().isPadronesIguales(supraElec); 
					};				
				};
