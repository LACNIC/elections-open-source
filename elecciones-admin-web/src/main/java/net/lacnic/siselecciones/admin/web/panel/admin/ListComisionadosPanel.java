...
public class ListComisionadosPanel extends Panel {
	...
	public ListComisionadosPanel(String id) {
		super(id);
		List<Comisionado> listaComisionados = Contexto.getInstance().getManagerBeanRemote().obtenerComisionados();
					...
					try {
						...
						BotonConConfirmacionEliminar botonConConfirmacionEliminar = new BotonConConfirmacionEliminar("eliminar",item.getIndex()) {
							...
							public void onConfirmar() {
								...
								try {
									Contexto.getInstance().getManagerBeanRemote().eliminarComisionado(actual.getIdComisionado(), actual.getNombre(), SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
