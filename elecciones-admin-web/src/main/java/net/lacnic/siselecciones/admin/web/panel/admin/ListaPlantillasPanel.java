...
import net.lacnic.siselecciones.admin.dashboard.admin.DashboardEditarPlantilla;
...
public class ListaPlantillasPanel extends Panel {
	...
	public ListaPlantillasPanel(String id, long idEleccion) {
		super(id);
		List<TemplateEleccion> listPlantillasBase = Contexto.getInstance().getManagerBeanRemote().obtenerTemplatesEleccion(idEleccion);
		Collections.sort(listPlantillasBase, new Comparator<TemplateEleccion>() {
			...
			public int compare(TemplateEleccion o1, TemplateEleccion o2) {
				return o1.getTipoTemplate().equals(Constantes.TipoTemplateNUEVO) ? -1 : o2.getTipoTemplate().equals(Constantes.TipoTemplateNUEVO) ? 1 : o1.getTipoTemplate().compareTo(o2.getTipoTemplate());
			}
		});
		...
		try {
			final ListView<TemplateEleccion> dataViewTemplates = new ListView<TemplateEleccion>("listaPlantillas", plantillasBase) {
				...
				private boolean botonEnviarVisible(TemplateEleccion actual) {
					return (!(actual.getTipoTemplate().equals(Constantes.TipoTemplateAUDITOR_CONFORME) || actual.getTipoTemplate().equals(Constantes.TipoTemplateCODIGOS_VOTACION) || actual.getTipoTemplate().equals(Constantes.TipoTemplateAUDITOR_REVISION)));
				}
			};
