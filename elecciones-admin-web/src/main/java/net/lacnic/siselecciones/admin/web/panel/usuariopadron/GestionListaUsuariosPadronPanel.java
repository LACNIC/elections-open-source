...
import net.lacnic.siselecciones.admin.dashboard.admin.DashboardEditarUsuarioPadron;
import net.lacnic.siselecciones.admin.dashboard.admin.DashboardGestionPadron;
import net.lacnic.siselecciones.admin.web.commons.BotonActualizarToken;
import net.lacnic.siselecciones.admin.web.commons.BotonConConfirmacionEliminar;
import net.lacnic.siselecciones.admin.web.commons.BotonReenviarEmailVotacion;
...
public class GestionListaUsuariosPadronPanel extends Panel {
	...
	public GestionListaUsuariosPadronPanel(String id, Eleccion eleccion) {
		super(id);
		usuariosPadron = Contexto.getInstance().getManagerBeanRemote().obtenerUsuariosPadronEleccion(eleccion.getIdEleccion());
		...
		DownloadLink downloadLink = new DownloadLink("exportarPadron", new PropertyModel<>(GestionListaUsuariosPadronPanel.this, "archivo")) {
			...
			@Override
			public void onClick() {
				setArchivo(Contexto.getInstance().getManagerBeanRemote().exportarPadronElectoral(eleccion.getIdEleccion()));
				super.onClick();
			}
		};
		add(downloadLink);
				...
				try {
					...
					BotonActualizarToken botonActualizarToken = new BotonActualizarToken("actualizar") {
						...
						public void onConfirmar() {
							...
							try {
								Contexto.getInstance().getManagerBeanRemote().actualizarTokenUsuarioPadron(actual.getIdUsuarioPadron(), actual.getNombre(), eleccion.getTituloEspanol(), SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
					...
					BotonReenviarEmailVotacion botonReenviarEmailVotacion = new BotonReenviarEmailVotacion("enviarLink") {
						...
						public void onConfirmar() {
							...
							try {
								Contexto.getInstance().getManagerBeanRemote().reenviarEmailPadron(actual, eleccion, SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
					...
					BotonConConfirmacionEliminar botonConConfirmacionEliminar = new BotonConConfirmacionEliminar("eliminar",item.getIndex()) {
						...
						public void onConfirmar() {
							...
							try {
								Contexto.getInstance().getManagerBeanRemote().eliminarUsuarioPadron(actual, eleccion.getTituloEspanol(), SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
