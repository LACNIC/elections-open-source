...
import net.lacnic.siselecciones.admin.web.commons.TipoTemplateValidator;
...
public class AddTemplateBasePanel extends Panel {
	...
	private TemplateEleccion template;

	public AddTemplateBasePanel(String id) {
		super(id);
		template = new TemplateEleccion();
		...
		form.add(new Button("guardar") {
			...
			public void onSubmit() {
				boolean agregarP = Contexto.getInstance().getManagerBeanRemote().agregarPlantillaBase(template, SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
				if (agregarP) {
