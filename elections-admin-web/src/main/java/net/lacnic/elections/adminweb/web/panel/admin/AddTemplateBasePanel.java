...
public class AddTemplateBasePanel extends Panel {
	...
	public AddTemplateBasePanel(String id) {
		...
		tipoTextField.add(new TemplateTypeValidator());
		...
		form.add(new Button("guardar") {
			...
			public void onSubmit() {
				boolean agregarP = AppContext.getInstance().getManagerBeanRemote().createBaseEmailTemplate(template, SecurityUtils.getUserAdminId(), SecurityUtils.getClientIp());
				if (agregarP) {
