...
public class ElectionCreatePanel extends Panel {

	private static final long serialVersionUID = 1L;

	public ElectionCreatePanel(String id, Election election) {
		super(id);
		...
		DropDownChoice<ElectionCategory> selectCategory = new DropDownChoice<>("selectCategory", new PropertyModel<>(election, "category"),
				SecurityUtils.getVisibleElectionCategories(), new ElectionCategoryChoiceRenderer(this));
		selectCategory.setRequired(true);
		add(selectCategory);

		DropDownChoice<ElectionType> electionType = new DropDownChoice<>("electionType", new PropertyModel<>(election, "electionType"), Arrays.asList(ElectionType.values()),
				new IChoiceRenderer<ElectionType>() {
			private static final long serialVersionUID = 1L;

			@Override
			public Object getDisplayValue(ElectionType object) {
				return object == null ? "" : getString("electionDeclarationsScope." + object.name());
			}
		...
		publicLinkRecoveryMode.setOutputMarkupId(true);
		publicLinkRecoveryMode.setNullValid(false);
		publicLinkRecoveryMode.setRequired(true);
		add(publicLinkRecoveryMode);

		electionType.add(new AjaxFormComponentUpdatingBehavior("change") {
			private static final long serialVersionUID = 1L;

			@Override
			protected void onUpdate(AjaxRequestTarget target) {
				ElectionType selectedType = election.getElectionType();
				ElectionLinkRecoveryMode defaultMode = ElectionLinkRecoveryMode.defaultForElectionType(selectedType);
				election.setPublicLinkRecoveryMode(defaultMode);
				target.add(publicLinkRecoveryMode);
			}
		});

		boolean campusConfigured = CampusClient.isCampusIntegrationEnabled();
		WebMarkupContainer campusCourses = createCampusCoursesContainer(campusConfigured);
		add(campusCourses);

		List<Course> courses = campusConfigured ? initCourse() : new ArrayList<>();
		Map<Long, String> campusCourseIdToName = buildCampusCourseLabelMap(courses);
		addCampusCourseChoice(campusCourses, "campusCourse", election, "campusCourse", campusCourseIdToName);
		addCampusCourseChoice(campusCourses, "campusCourseEnglish", election, "campusCourseEnglish", campusCourseIdToName);
		addCampusCourseChoice(campusCourses, "campusCoursePortuguese", election, "campusCoursePortuguese", campusCourseIdToName);

		String campusUrl = campusConfigured ? CampusClient.getCampusUrl() : "";
		campusUrl = campusUrl == null ? "" : campusUrl.trim();
		if (campusUrl.isEmpty()) {
			campusUrl = "-";
		}
		campusCourses.add(new Label("campusCourseHelp", new StringResourceModel("electionManagementCampusCourseHelp", this, null).setParameters(campusUrl)));
	...
	static WebMarkupContainer createCampusCoursesContainer(boolean campusConfigured) {
		WebMarkupContainer campusCourses = new WebMarkupContainer("campusCourses");
		campusCourses.setVisible(campusConfigured);
	...
	private void addCampusCourseChoice(WebMarkupContainer campusCourses, String componentId, Election election, String propertyName,
			Map<Long, String> campusCourseIdToName) {
		DropDownChoice<Long> campusCourse = new DropDownChoice<>(componentId, new PropertyModel<>(election, propertyName), new ArrayList<>(campusCourseIdToName.keySet()), new IChoiceRenderer<Long>() {
			...
			public Object getDisplayValue(Long object) {
				return object == null ? "" : campusCourseIdToName.getOrDefault(object, String.valueOf(object));
			}
		...
		campusCourses.add(campusCourse);
	}
	...
	private List<Course> initCourse() {
		List<Course> courses = CampusClient.getCourses();
		if (courses == null) {
			error(getString("electionManagementCampusConnectionError"));
