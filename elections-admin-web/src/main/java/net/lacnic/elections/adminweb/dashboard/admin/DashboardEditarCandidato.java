			...
			formCandidato.add(new Button("editarCandidato") {
				...
				private void cambiarDatos() {
					if (!(nombre.equalsIgnoreCase(candidato.getName())) || !(Arrays.equals(archivoFoto, candidato.getPictureInfo()))
						...
						AppContext.getInstance().getManagerBeanRemote().editCandidate(getCandidato(), SecurityUtils.getUserAdminId(), SecurityUtils.getClientIp());
						getSession().info(getString("candidateEditExito"));
					} else
						getSession().error(getString("candidateEditError"));

					setResponsePage(ElectionCandidatesDashboard.class, UtilsParameters.getId(candidato.getElection().getElectionId()));
				}
