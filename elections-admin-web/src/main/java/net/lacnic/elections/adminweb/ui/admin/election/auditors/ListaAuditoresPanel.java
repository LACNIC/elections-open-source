		...
		try {
			...
			ListView<Auditor> auditoresDataView = new ListView<Auditor>("auditoresList", auditores) {
				...
				protected void populateItem(final ListItem<Auditor> item) {
					...
					item.add(new ButtonDeleteWithConfirmation("eliminar",item.getIndex()) {
						...
						public void onConfirm() {
							...
							AppContext.getInstance().getManagerBeanRemote().removeAuditor(actual.getAuditorId(), SecurityUtils.getUserAdminId(), SecurityUtils.getClientIp());
							setResponsePage(ElectionAuditorsDashboard.class, UtilsParameters.getId(idEleccion));
						}
