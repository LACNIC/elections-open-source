...
public class AddUserAdminPanel extends Panel {
	...
	public AddUserAdminPanel(String id) {
		...
		DropDownEleccion dropDownElecciones = new DropDownEleccion("authorizedElection", new PropertyModel<>(AddUserAdminPanel.this, "authorizedElection"));
		...
		formUserAdmin.add(new Button("add") {
			...
			public void onSubmit() {
				...
				boolean result = AppContext.getInstance().getManagerBeanRemote().addUserAdmin(userAdmin, SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
				if (result) {
