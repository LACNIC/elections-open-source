...
public class EditUserAdminDashboard extends DashboardAdminBasePage {
	...
	public EditUserAdminDashboard(PageParameters params) {
		...
		DropDownEleccion dropDownElecciones = new DropDownEleccion("authorizedElection", new PropertyModel<>(EditUserAdminDashboard.this, "authorizedElection"));
		...
		formUserAdmin.add(new Button("edit") {
			...
			public void onSubmit() {
				...
				if (!(email.equalsIgnoreCase(userAdmin.getEmail()) && getAuthorizedElection().getElectionId() == userAdmin.getAuthorizedElectionId().longValue())) {
					AppContext.getInstance().getManagerBeanRemote().editUserAdmin(userAdmin, email, getAuthorizedElection().getElectionId(), SecurityUtils.getAdminId(), SecurityUtils.getIPClient());
